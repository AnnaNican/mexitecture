 ...
 */(function(PolymerVis){/**
   ...
   */PolymerVis.loadScript=function(src,onload,onerror,optAsync=!0){var ele=document.createElement("script");ele.src=src;ele.async=optAsync;if(onload)ele.onload=onload;if(onerror)ele.onerror=onerror;document.body.appendChild(ele);return ele};/**
   ...
   */PolymerVis.loadStylesheet=function(href,onload,onerror,optAsync=!0){var ele=document.createElement("link");ele.href=href;ele.rel="stylesheet";ele.async=optAsync;if(onload)ele.onload=onload;if(onerror)ele.onerror=onerror;document.body.appendChild(ele);return ele};/**
   ...
   */PolymerVis.insertCssIntoShadowRoot=function insertCssIntoShadowRoot(cssSrc,shadowRoot,onload,id="custom"){var httpRequest=new XMLHttpRequest;if(!httpRequest){console.warn(`cannot create XMLHttpRequest to load ${cssSrc}`);return}httpRequest.onreadystatechange=()=>{if(httpRequest.readyState===XMLHttpRequest.DONE){if(200===httpRequest.status){var style=shadowRoot.querySelector(`#${id}`)||document.createElement("style");style.id=id;style.textContent=httpRequest.responseText;shadowRoot.appendChild(style);if(onload)onload(style)}else{console.warn(`cannot load ${cssSrc}`)}}};httpRequest.open("GET",cssSrc);httpRequest.send()};/**
